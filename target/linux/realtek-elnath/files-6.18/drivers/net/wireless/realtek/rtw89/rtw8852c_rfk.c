...
/* ---- RTL9607F "Elnath" ONU, 8852CE RFE type 53 external-FEM (KCT8539HE) ----
 *
 * Upstream rtw8852c's RFK is internal-FEM only.  For the eFEM 5 GHz PA to
 * deliver full power TWO things upstream lacks are needed, on TOP of routing
 * the RFE control pads to the FEM (see rtw8852c_rfe53_pinmux() in rtw8852c.c):
 *
 *  1. the eFEM TSSI/RFE BB config: external-TSSI detector select
 *     (0x5820/0x7820 bit30), fixed-slope disable (0x580c/0x780c bit28/30 = 0),
 *     the per-channel slope, and the FEM PAPE/TRSW output-pin mux.  Sourced
 *     from this board's stock flash "MP" calibration (captured live off the
 *     running device at full TX power, 2026-07-16) because mainline cannot
 *     read that flash partition and the WiFi efuse is blank.  Applied AFTER
 *     upstream's TSSI setup so it wins over the iFEM defaults.
 *  2. a higher TSSI power-servo target (DE): with the RFE pads routed the FEM
 *     PA has full headroom, but this board's TSSI detector reads high, so with
 *     the stock DE the closed loop settles ~30 dB low; DE = 0x2a0 makes the
 *     loop servo to full power and HOLD (live-verified stable -13 dBm at the
 *     reference antenna, vs stock -10; without the RFE-pad pinmux it collapses
 *     to -63, proving the pinmux is the load-bearing enable).
 *
 * RF-side eTSSI params + the FEM TX-gain tracking LUT come from the stock
 * RFE53/KCT8539HE parameter set.
 */
static const u32 rtw8852c_rfe53_efem_rf[][2] = {
	{0x000df, 0x00200}, {0x000e0, 0x80000}, {0x0006a, 0xe0380}, {0x0006b, 0x03ca0},
	{0x0006f, 0xc01fc}, {0x0005e, 0x0001f}, {0x100ee, 0x02000}, {0x10030, 0x200e8},
	{0x10030, 0x204e5}, {0x10030, 0x208e2}, {0x10030, 0x20cdf}, {0x10030, 0x210dc},
	{0x10030, 0x214d9}, {0x10030, 0x218d6}, {0x10030, 0x21cd3}, {0x10030, 0x220d0},
	{0x10030, 0x224cd}, {0x10030, 0x228cd}, {0x10030, 0x22ccd}, {0x10030, 0x230cd},
	{0x10030, 0x234cd}, {0x10030, 0x238cd}, {0x10030, 0x23ccd}, {0x10030, 0x240cd},
	{0x10030, 0x280e8}, {0x10030, 0x284e5}, {0x10030, 0x288e2}, {0x10030, 0x28cdf},
	{0x10030, 0x290dc}, {0x10030, 0x294d9}, {0x10030, 0x298d6}, {0x10030, 0x29cd3},
	{0x10030, 0x2a0d0}, {0x10030, 0x2a4cd}, {0x10030, 0x2a8cd}, {0x10030, 0x2accd},
	{0x10030, 0x2b0cd}, {0x10030, 0x2b4cd}, {0x10030, 0x2b8cd}, {0x10030, 0x2bccd},
	{0x10030, 0x2c0cd}, {0x10030, 0x300e8}, {0x10030, 0x304e5}, {0x10030, 0x308e2},
	{0x10030, 0x30cdf}, {0x10030, 0x310dc}, {0x10030, 0x314d9}, {0x10030, 0x318d6},
	{0x10030, 0x31cd3}, {0x10030, 0x320d0}, {0x10030, 0x324cd}, {0x10030, 0x328cd},
	{0x10030, 0x32ccd}, {0x10030, 0x330cd}, {0x10030, 0x334cd}, {0x10030, 0x338cd},
	{0x10030, 0x33ccd}, {0x10030, 0x340cd}, {0x10030, 0x600e8}, {0x10030, 0x604e5},
	{0x10030, 0x608e2}, {0x10030, 0x60cdf}, {0x10030, 0x610dc}, {0x10030, 0x614d9},
	{0x10030, 0x618d6}, {0x10030, 0x61cd3}, {0x10030, 0x620d0}, {0x10030, 0x624cd},
	{0x10030, 0x628cd}, {0x10030, 0x62ccd}, {0x10030, 0x630cd}, {0x10030, 0x634cd},
	{0x10030, 0x638cd}, {0x10030, 0x63ccd}, {0x10030, 0x640cd}, {0x10030, 0x680e8},
	{0x10030, 0x684e5}, {0x10030, 0x688e2}, {0x10030, 0x68cdf}, {0x10030, 0x690dc},
	{0x10030, 0x694d9}, {0x10030, 0x698d6}, {0x10030, 0x69cd3}, {0x10030, 0x6a0d0},
	{0x10030, 0x6a4cd}, {0x10030, 0x6a8cd}, {0x10030, 0x6accd}, {0x10030, 0x6b0cd},
	{0x10030, 0x6b4cd}, {0x10030, 0x6b8cd}, {0x10030, 0x6bccd}, {0x10030, 0x6c0cd},
	{0x10030, 0x700e8}, {0x10030, 0x704e5}, {0x10030, 0x708e2}, {0x10030, 0x70cdf},
	{0x10030, 0x710dc}, {0x10030, 0x714d9}, {0x10030, 0x718d6}, {0x10030, 0x71cd3},
	{0x10030, 0x720d0}, {0x10030, 0x724cd}, {0x10030, 0x728cd}, {0x10030, 0x72ccd},
	{0x10030, 0x730cd}, {0x10030, 0x734cd}, {0x10030, 0x738cd}, {0x10030, 0x73ccd},
	{0x10030, 0x740cd}, {0x10030, 0x780e8}, {0x10030, 0x784e5}, {0x10030, 0x788e2},
	{0x10030, 0x78cdf}, {0x10030, 0x790dc}, {0x10030, 0x794d9}, {0x10030, 0x798d6},
	{0x10030, 0x79cd3}, {0x10030, 0x7a0d0}, {0x10030, 0x7a4cd}, {0x10030, 0x7a8cd},
	{0x10030, 0x7accd}, {0x10030, 0x7b0cd}, {0x10030, 0x7b4cd}, {0x10030, 0x7b8cd},
	{0x10030, 0x7bccd}, {0x10030, 0x7c0cd}, {0x10030, 0x00000}, {0x000fd, 0x15372},
};
...
static const struct rtw89_reg2_def rtw8852c_rfe53_efem_bb[] = {
	...
	{0x580c, 0xa0004040}, {0x5810, 0x59008000}, {0x5814, 0x2012f000},
	{0x581c, 0x3dc80280}, {0x5820, 0xc0000080}, {0x5824, 0x0000011d},
	{0x5828, 0x100130df}, {0x582c, 0x0000011d}, {0x5830, 0x100130df},
	{0x5834, 0x0000011d}, {0x5838, 0x100130df}, {0x583c, 0x00000136},
	{0x5840, 0x100130dc}, {0x5844, 0x0000012b}, {0x5848, 0x100130df},
	{0x584c, 0x0000012f}, {0x5850, 0x100130e2}, {0x5858, 0x02130121},
	{0x5860, 0x02130121}, {0x5864, 0x1a0801ff}, {0x5868, 0xa9a90002},
	{0x5880, 0x30777777}, {0x5884, 0x77777477}, {0x5890, 0x00000c80},
	{0x5894, 0x010a0604}, {0x58d8, 0x800801ff}, {0x58e8, 0x18000004},
	{0x780c, 0xa0004040}, {0x7810, 0x59007c00}, {0x7814, 0x2012f000},
	{0x781c, 0x3dc80280}, {0x7820, 0xc0000080}, {0x7824, 0x0000010f},
	{0x7828, 0x100130df}, {0x782c, 0x0000010f}, {0x7830, 0x100130df},
	{0x7834, 0x0000010f}, {0x7838, 0x100130df}, {0x783c, 0x00000127},
	{0x7840, 0x100130dc}, {0x7844, 0x00000119}, {0x7848, 0x100130df},
	{0x784c, 0x00000120}, {0x7850, 0x100130e2}, {0x7858, 0x02130121},
	{0x7860, 0x02130121}, {0x7864, 0x19f801ff}, {0x7868, 0xa9a90002},
	{0x7880, 0x77770347}, {0x7884, 0x77777777}, {0x7890, 0x00000006},
	{0x7894, 0x010a0604}, {0x78d8, 0x800801ff}, {0x78e8, 0x18000004},
};

static const u32 rtw8852c_rfe53_de_regs[] = {
 ...
 */
static struct rtw89_dev *rtw8852c_rfe53_kick_dev;
static int rtw8852c_rfe53_kick_stage;
static void rtw8852c_rfe53_kick_fn(struct work_struct *work);
static DECLARE_DELAYED_WORK(rtw8852c_rfe53_kick_dwork, rtw8852c_rfe53_kick_fn);

static void rtw8852c_rfe53_de_write(struct rtw89_dev *rtwdev, u32 de)
{
	u32 i;

	for (i = 0; i < ARRAY_SIZE(rtw8852c_rfe53_de_regs); i++)
		rtw89_phy_write32_mask(rtwdev, rtw8852c_rfe53_de_regs[i],
				       GENMASK(21, 12), de);
}

/* Two-stage worker: stage 0 re-establishes the low servo target DE = 0x130
 * (defensive: rtw8852c_tssi_scan()'s _tssi_set_efuse_to_de may have moved it),
...
static void rtw8852c_rfe53_kick_fn(struct work_struct *work)
{
	struct rtw89_dev *rtwdev = rtw8852c_rfe53_kick_dev;

	if (!rtwdev || rtwdev->efuse.rfe_type != 53)
		return;

	wiphy_lock(rtwdev->hw->wiphy);
	if (rtw8852c_rfe53_kick_stage == 0) {
		rtw8852c_rfe53_de_write(rtwdev, 0x130);
		rtw8852c_rfe53_kick_stage = 1;
		mod_delayed_work(system_wq, &rtw8852c_rfe53_kick_dwork,
				 msecs_to_jiffies(6000));
	} else {
		rtw8852c_rfe53_de_write(rtwdev, 0x2a0);
	}
...
static void _rfe53_efem_apply(struct rtw89_dev *rtwdev)
{
	...
	for (path = RF_PATH_A; path <= RF_PATH_B; path++)
		for (i = 0; i < ARRAY_SIZE(rtw8852c_rfe53_efem_rf); i++)
			rtw89_write_rf(rtwdev, path,
				       rtw8852c_rfe53_efem_rf[i][0], RFREG_MASK,
				       rtw8852c_rfe53_efem_rf[i][1]);

	for (i = 0; i < ARRAY_SIZE(rtw8852c_rfe53_efem_bb); i++)
		rtw89_phy_write32(rtwdev, rtw8852c_rfe53_efem_bb[i].addr,
				  rtw8852c_rfe53_efem_bb[i].data);

	/* Arm the settle-then-step worker ~10 s out (once the AP is beaconing). */
	rtw8852c_rfe53_kick_dev = rtwdev;
	rtw8852c_rfe53_kick_stage = 0;
	mod_delayed_work(system_wq, &rtw8852c_rfe53_kick_dwork,
