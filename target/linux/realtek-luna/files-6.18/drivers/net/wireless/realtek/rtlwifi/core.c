			 ...
			 struct ieee80211_sta *sta)
{
	...
	if (sta) {
		...
		rtl_dbg(rtlpriv, COMP_MAC80211, DBG_DMESG,
			"Add sta addr is %pM\n", sta->addr);
		/* TEMP AP-PS probe: the STA's negotiated U-APSD (0 = legacy PS-Poll
		 * path -> needs the live-TIM beacon; non-zero -> uAPSD unicast path). */
		pr_info("rtl92fe sta_add %pM wme=%d uapsd=%#x max_sp=%u\n",
			sta->addr, sta->wme, sta->uapsd_queues, sta->max_sp);
				...
				struct ieee80211_sta *sta)
{
	...
	if (sta) {
		...
		/* P1d: defensively release any HW CAM key slot this STA still holds,
		 * so slot lifetime is bounded by STA lifetime even when a DISABLE_KEY
		 * was missed (hostapd SIGKILL, error path, teardown race) -- that
		 * missed-del is what leaks slots until a reboot. Idempotent: a no-op
		 * once already freed. */
		if (vif->type == NL80211_IFTYPE_AP ||
		    vif->type == NL80211_IFTYPE_MESH_POINT) {
			u8 cam_i;

			for (cam_i = 4; cam_i < TOTAL_CAM_ENTRY; cam_i++)
				if (ether_addr_equal_unaligned(
				    rtlpriv->sec.hwsec_cam_sta_addr[cam_i],
				    sta->addr)) {
					rtl_cam_delete_one_entry(hw, sta->addr,
								 cam_i);
					break;
				}
			       ...
			       struct ieee80211_ampdu_params *params)
{
	...
	case IEEE80211_AMPDU_TX_START:
		 ...
		 * complete fine), then the Block-Ack session turns operational,
		 * frames switch to AMPDU, and the HW stops completing descriptors
		 * (hw_idx freezes -> no DOK -> ring fills -> stop-queue latch). The
		 * mgmt/EAPOL frames never open a BA session, which is exactly why
		 * they always transmitted while data died after a few frames,
		 * independent of rate/crypto. Declining the BA keeps all data on the
		 * proven non-aggregated path so it flows continuously. Costs peak
		 * throughput only; re-enable once the aggregated-BD path is fixed. */
		rtl_dbg(rtlpriv, COMP_MAC80211, DBG_TRACE,
			"IEEE80211_AMPDU_TX_START refused (agg-BD path wedges HW)\n");
		return -EOPNOTSUPP;
			  ...
			  struct ieee80211_key_conf *key)
{
	...
	case SET_KEY:
		...
		/* must use sw generate IV, or can not work !!!! -- confirmed 2026-07-06:
		 * PUT_IV_SPACE (HW-owned PN) regressed CCMP TX to no-radiation. So the
		 * downstream-undecodable bug is NOT the flag; it is the 8192F not
		 * consuming the SW IV's PN at encrypt time (an 8192F-specific config /
		 * descriptor IV-offset), still under investigation. */
	...
	case DISABLE_KEY:
		...
		if (vif->type == NL80211_IFTYPE_AP ||
			...
			if (sta) {
				...
				/* P1a: wipe the REAL dynamic HW CAM slot (4..31) that held
				 * this STA's pairwise key. The rtl_cam_delete_one_entry(...,
				 * key_idx) below only clears entry key_idx (a default-key
				 * slot for a pairwise key), so without this a valid HW entry
				 * carrying the STA's MAC + a dead PTK lingers = the per-STA
				 * key-slot leak. Resolve the slot from the SW mirror BEFORE
				 * rtl_cam_del_entry clears the mirror. */
				for (cam_i = 4; cam_i < TOTAL_CAM_ENTRY; cam_i++)
					if (ether_addr_equal_unaligned(
					    rtlpriv->sec.hwsec_cam_sta_addr[cam_i],
					    sta->addr)) {
